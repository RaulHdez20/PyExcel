...
fecha_inicio = datetime(2025, 6, 8, 21, 0)  #Primeros tres datos para fecha y los ultimos 2 para hora
fecha_fin = datetime(2025, 6, 8, 23, 0)  #Primeros tres datos para fecha y los ultimos 2 para hora
...
archivo_modificado = "Resumen_Piezas2.5.xlsx"
wb = openpyxl.Workbook()
ws = wb.active
ws.title = "Piezas por Intervalo"
ws.append(["total", "fecha inicio", "fecha fin"])

# Escribir los datos procesados en el formato deseado
for _, row in grupo_por_intervalo.iterrows():
    ws.append([row["Piezas_Hechas"], row["Fecha Inicio"].strftime("%d/%m/%Y %H:%M:%S"), row["Fecha Fin"].strftime("%d/%m/%Y %H:%M:%S")])

wb.save(archivo_modificado)
print(f"\nArchivo guardado como: {archivo_modificado}")